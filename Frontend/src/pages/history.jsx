      ...
      <div style={{ minHeight: "100vh", background: "var(--bg-primary)" }}>
        ...
        <header style={{
          ...
          <span style={{
            ...
            {historyData.length} translations
          </span>
        </header>
        ...
        <main style={{ padding: "28px", display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          {loading ? (
            ...
            <>
              ...
              <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                {[
                  { label: "Today", value: historyData.filter(h => Math.floor((new Date() - new Date(h.created_at)) / (1000 * 60 * 60 * 24)) === 0).length, color: "var(--accent)" },
                  { label: "Yesterday", value: historyData.filter(h => Math.floor((new Date() - new Date(h.created_at)) / (1000 * 60 * 60 * 24)) === 1).length, color: "#22c55e" },
                  { label: "This Week", value: historyData.filter(h => Math.floor((new Date() - new Date(h.created_at)) / (1000 * 60 * 60 * 24)) <= 7).length, color: "#f59e0b" },
